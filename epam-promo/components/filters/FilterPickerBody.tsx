    ...
    renderBody(props: DataSourceListProps & Omit<PickerBodyBaseProps, 'rows'>, rows: DataRowProps<TItem, TId>[]) {
        ...
        return (
            <Panel style={{ width: minBodyWidth }} rawProps={{ tabIndex: -1 }} cx={[uuiMarkers.lockFocus]}>
                <MobileDropdownWrapper title={this.props.entityName} close={() => this.toggleBodyOpening(false)}>
                    <DataPickerBody {...props} rows={renderedDataRows} maxHeight={maxHeight} searchSize="36" editMode="dropdown" showSearch={true} />
