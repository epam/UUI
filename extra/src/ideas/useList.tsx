...
type UseListProps<TItem, TId, TFilter> =
    ...
    } & (Partial<IEditable<TItem[]>> | Partial<IEditable<Tree<TItem, TId>>>); // Let's try to use IEditable directly: // We also need to pass metadata and validationState along with the value. It seems like we are forced to use IEditable here. // // - tree: Tree<TItem, TId> // convenient and efficient. But not serializable, and we need to adjust metadata/validate/lenses to support it // - [maybe] byId: Map<TId, TItem> // a bit more efficient and convenient. However, it can't store the order of items. And we'd need to re-build the tree anyway // - items: TItem[] // simple, backward-compatible. Tree manipulation is hard, and we need to re-build the tree on each change // We want to support several formats: // // We need to decide on how to put items/setItems into useList
