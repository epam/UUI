...
function Languages({ lens }: { lens: ILens<PersonDetails['languageInfo']> }) {
    ...
    return (
        <>
            ...
            { lens.get().map(({
                language, speakingLevel, writingLevel, 
            }, index) => {
...
function Visas({ lens, countriesDS }: { lens: ILens<PersonDetails['travelVisas']>, countriesDS: AsyncDataSource<Country, string, unknown> }) {
    ...
    return (
        <>
                    ...
                    >
                        ...
                        >
                            { isClearable && (
                                <IconButton
                                    icon={ clearIcon }
                                    onClick={ () => removeLensItemHandler<PersonTravelVisa>(visasLens, index) }
                                />
                            ) }
                        </FlexRow>
                    </FlexRow>
            ...
            <FlexRow vPadding="12">
                <Button
                    onClick={ () => addLensItemHandler<PersonTravelVisa>(visasLens, emptyInfo.visa) }
                    caption="Add One More"
                    icon={ addIcon }
                    fill="none"
                />
