...
export function getDemoApi(
    processRequest: (request: string, requestMethod: string, data?: any, options?: ApiCallOptions) => any,
    origin: string = ''
) {
    ...
    return {
        ...
        personsPaged: (rq: LazyDataSourceApiRequest<models.Person, number, DataQueryFilter<models.Person>> & { page: number; pageSize: number }) =>
            processRequest(origin.concat('/api/persons-paged'), 'POST', rq) as Promise<
                LazyDataSourceApiResponse<models.Person> & { totalCount: number }
            >,
