...
export const basicPickerTogglerDoc = new DocBuilder<IBasicPickerToggler>({ name: 'onClear' }).prop('onClear', {
    examples: ctx => [ctx.getCallback('onClear')],
});
...
export const isDisabledDoc = new DocBuilder<IDisableable>({ name: 'Font' }).prop('isDisabled', {
    description: "Disabled controls doesn't emit events like onClick",
    examples: [true],
});

export const isReadonlyDoc = new DocBuilder<ICanBeReadonly>({ name: 'Font' }).prop('isReadonly', { examples: [true] });

export const isInvalidDoc = new DocBuilder<ICanBeInvalid>({ name: 'isInvalid' }).prop('isInvalid', { examples: [true] });
