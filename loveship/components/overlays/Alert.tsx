...
export const Alert = React.forwardRef<HTMLDivElement, AlertProps>((props, ref) => (
    <div role="alert" ref={ref} className={cx(css.alertWrapper, css[`color-${props.color || 'sky'}`], props.cx)} {...props.rawProps}>
                ...
                {props.actions && (
                    <div className={css.actionWrapper}>
                        {props.actions.map(action => (
                            <LinkButton caption={action.name} onClick={action.action} key={action.name} cx={css.actionLink} size="24" fontSize="14" />
                        ))}
                    </div>
                )}
            </div>
