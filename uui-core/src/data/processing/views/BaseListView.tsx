...
import {
    ...
    IDataSourceView, DataRowPathItem, CascadeSelectionTypes, GetSelectedRowsOptions,
} from "../../../types";
...
export abstract class BaseListView<TItem, TId, TFilter> implements IDataSourceView<TItem, TId, TFilter> {
    ...
    public getSelectedRows = ({ start = 0, end }: GetSelectedRowsOptions = {}) => {
        let checked: TId[] = [];
        ...
        if (end !== undefined) {
            checked = checked.slice(start, end);
        }

        return checked.map((id, n) => this.getById(id, start + n));
    }
