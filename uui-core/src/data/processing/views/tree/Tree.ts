...
export class Tree<TItem, TId, TSubtotals = void> extends LoadableTree<TItem, TId, TSubtotals> {
    public filter<TFilter>(options: ApplyFilterOptions<TItem, TId, TFilter>): ITree<TItem, TId, TSubtotals> {
        const filter = options.getFilter?.(options.filter);
        return this.applyMatchToTree(filter);
    }

    public search<TFilter>(options: ApplySearchOptions<TItem, TId, TFilter>): ITree<TItem, TId, TSubtotals> {
    ...
    public sort<TFilter>(options: ApplySortOptions<TItem, TId, TFilter>) {
        ...
        return Tree.create<TItem, TId, TSubtotals>({ ...this.params }, sortedItems, this.subtotals);
    }
    ...
    private applyMatchToTree(isMatchingFn: undefined | ((item: TItem) => boolean)): ITree<TItem, TId, TSubtotals> {
        ...
        return Tree.create<TItem, TId, TSubtotals>({ ...this.params }, matchedItems, this.subtotals);
    }
}
