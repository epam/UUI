...
describe('useForm', () => {
    ...
    describe('Basic updates handing', () => {
        it('Should update form value with onValueChange', async () => {
            const { result } = await mountHookWithContext<UseFormProps<string>, IFormApi<string>>(() =>
                useForm({
        ...
        it('Should update form value with setValue (callback)', async () => {
            const { result } = await mountHookWithContext<UseFormProps<number>, IFormApi<number>>(() =>
                useForm({
                    onSave: () => Promise.resolve(),
                    onError: () => Promise.resolve(),
                    value: 1,
        ...
        it('should update form value by external props.value change', async () => {
            const { result, rerender } = await mountHookWithContext<UseFormProps<number>, IFormApi<number>>(props => useForm(props), {
                onSave: () => Promise.resolve(),
                value: 1,
            });
    ...
    describe('Client validation', () => {
        it('Should return isChanged as true whenever the lens is changed', async () => {
            const { result } = await mountHookWithContext<UseFormProps<IFoo>, IFormApi<IFoo>>(() =>
                useForm({
                    onSave: () => Promise.resolve(),
                    onError: () => Promise.resolve(),
                    value: testData,
    ...
    describe('isChanged, redo/undo/revert handing', () => {
        ...
        it('Should reset lock after component unmount', async () => {
            const beforeLeaveMock = jest.fn().mockResolvedValue(false);
            const { result, unmount } = await mountHookWithContext<UseFormProps<IFoo>, IFormApi<IFoo>>(() =>
                useForm({
                    value: testData,
                    onSave: () => Promise.resolve(),
                    beforeLeave: beforeLeaveMock,
        ...
        it('Should restore data from local storage after leaving form without saving changes', async () => {
            ...
            const { result: secondRenderResult } = await mountHookWithContext<UseFormProps<IFoo>, IFormApi<IFoo>>(() =>
                useForm({
                    ...props,
    ...
    describe('useForm Server validation', () => {
        ...
        it('Should correctly handle server validation', async () => {
            ...
            const { result: firstResult, unmount } = await mountHookWithContext<UseFormProps<IAdvancedFoo>, IFormApi<IAdvancedFoo>>(() =>
                useForm({
                    value: testData,
                    onSave: data => Promise.resolve({ form: data }),
                    onSuccess: () => '',
            ...
            const { result: secondResult } = await mountHookWithContext<UseFormProps<IAdvancedFoo>, IFormApi<IAdvancedFoo>>(() =>
                useForm({
                    value: testData,
        ...
        it('Should keep server error notification until field is changed', async () => {
            ...
            const { result } = await mountHookWithContext<UseFormProps<IAdvancedFoo>, IFormApi<IAdvancedFoo>>(() =>
                useForm({
