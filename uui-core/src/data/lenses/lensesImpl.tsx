...
export function getById<TItem, TId>(id: TId): ILensImpl<ITree<TItem, TId>, TItem> {
    return {
        get(big: ITree<TItem, TId>) {
            if (!big) {
                return undefined;
            } else {
                return big.getById(id);
            }
        },
        set(big: ITree<TItem, TId>, small: TItem) {
            return big.patch([small]);
        },
        getValidationState(big: ICanBeInvalid) {
            let validationStateProps = (big || blankValidationState).validationProps || {};
        ...
        getMetadata(big: Metadata<ITree<TItem, TId>>) {
            ...
            let metadataProps = metadata.props;
            let { isDisabled, isRequired, isReadonly } = metadata;
            return { ...metadataProps, isDisabled, isReadonly, isRequired } as any;
        },
    };
