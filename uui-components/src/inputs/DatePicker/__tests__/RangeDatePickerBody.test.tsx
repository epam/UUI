...
describe('DatePickerBody', () => {
    it('[focusPart:from] should clear "To" only when user selects "From" which is bigger than "To"', async () => {
        const { result, mocks } = await setupRangePickerBody({
            focusPart: 'from',
        ...
        // case-1
        const [oct13, nov13] = result.queryAllByText('13');
        ...
        // case-2
        const [oct17, nov17] = result.queryAllByText('17');
        ...
        const [oct18, nov18] = result.queryAllByText('18');
    ...
    it('[focusPart:to] should select "From" when user selects "To" which is less than "From"', async () => {
        ...
        // case-1
        const [oct16, nov16] = result.queryAllByText('16');
        ...
        // case-2
        const [oct12, nov12] = result.queryAllByText('12');
        ...
        const [oct11, nov11] = result.queryAllByText('11');
    ...
    it('should change view by click on title', async () => {
        const { result, mocks, dom } = await setupRangePickerBody({
            focusPart: 'from',
            selectedDate: { from: '2019-10-12', to: '2019-10-17' },
        });
        expect(dom.title.innerHTML).toEqual('October 2019');
        fireEvent.click(dom.title);
        expect(result.queryByText('2019')).toBeTruthy();
        expect(result.queryByText('Oct').classList.contains('uui-monthselection-current-month')).toBeTruthy();
        fireEvent.click(dom.title);
        expect(result.queryByText('October 2019')).toBeTruthy();
        expect(result.queryByText('2019').classList.contains('uui-yearselection-current-year')).toBeTruthy();
    });
