 ...
 */
function filterOutStylesNotReadyForRender(styles: CSSProperties, shouldRender: boolean) {
    if (shouldRender) {
        return styles;
    }
...
export const ScrollBars = forwardRef<ScrollbarsApi, ScrollbarProps>(({
    ...
    const renderSafeForSsr = {
        renderView: ({ style }: { style: CSSProperties }) => {
            const styleFiltered = filterOutStylesNotReadyForRender(style, shouldRender);
            if (props.renderView) {
