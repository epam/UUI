...
import { getNewColumnOrder, findFirstByGroupKey, findLastByGroupKey, isNonEmptyString } from "./columnsConfigurationUtils";
...
export function toggleAllColumnsVisibility(props: { prevConfig: ColumnsConfig, columns: DataColumnProps[], value: boolean }) {
    ...
    return Object.keys(prevConfig).reduce<ColumnsConfig>((acc, key) => {
        ...
        const noChangeRequired = isAlreadyToggled || tryingToHideAlwaysVisible || !isNonEmptyString(c.caption as string);
        if (noChangeRequired) {
            acc[key] = prevCfg;
        } else {
