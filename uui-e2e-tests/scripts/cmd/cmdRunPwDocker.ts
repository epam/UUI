...
function getVolumesMapArgs() {
    const absPathToPreviewIds = forwardSlashes(path.resolve('../uui-docs/src/docItems/_types/previewIds.ts'));
    ...
    return [
        ...
        return acc;
    }, []).concat(['-v', `${absPathToPreviewIds}:/uui-docs/src/docItems/_types/previewIds.ts`]);
}
