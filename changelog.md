...
* [useVirtualList]: Added optional `virtualRowInfo` prop. When `height` is provided, skips `getBoundingClientRect` measurement; when `gap` is provided, uses it for vertical spacing between rows (margins, flex/grid gap). Improves scroll performance on lists with uniform row sizing.
...
**What's Fixed**
* [useVirtualList]: Fixed scroll jump when the visible range changes (e.g. topIndex 0→20). Row height now includes top/bottom margins, preventing offset drift ([#2863](https://github.com/epam/UUI/issues/2863))
...
**What's Fixed**
* [RangeDatePicker]: clicking on a disabled date clears the input ([#3017](https://github.com/epam/UUI/issues/3017))
...
* [TimePicker]: fixed an unnecessary side effect for onValueChange if the Dropdown was closed with a default state ([#3015](https://github.com/epam/UUI/issues/3015))
* [ScrollBars]: added `elementEvents` property ([#2956](https://github.com/epam/UUI/issues/2956)).
...
* [SliderRating]: fixed issue where clicking before slider start could set '0' value, bypassing 'n/a' button restriction when `withoutNa` prop is true ([#2768](https://github.com/epam/UUI/issues/2768))
* [SliderRating]: fix tooltip behavior in read-only mode ([#2500](https://github.com/epam/UUI/issues/2500))
* [Date Picker]: prevent redundant DatePicker value updates on blur which caused tab order cycling inside dropdowns ([#2951](https://github.com/epam/UUI/issues/2951))
...
* [Inputs/Controls]:Made clickable icons and clear buttons accessible via keyboard ([#2845](https://github.com/epam/UUI/pull/2845)). Now it's possible to focus on them during keyboard navigation.
...
* [MainMenu]: Improved accessibility of hidden items in the "More" menu ([#2948](https://github.com/epam/UUI/pull/2948)).
...
* [PickerInput]: Changed value in `aria-haspopup` attribute from "true" to "dialog" ([#2947](https://github.com/epam/UUI/pull/2947)).
...
* [Pagination]: Fixed an issue where pagination elements were not grouped into a list ([#2942](https://github.com/epam/UUI/pull/2942)).
    * Added "Previous page" and "Next page" `aria-label`s to the previous and next pages respectively.
...
* [PickerInput/PickerModal]: Fixed announcing of a picker item's content in screen readers when the item becomes focused ([#2946](https://github.com/epam/UUI/pull/2946)).
