...
describe("db - DB queries and updates", () => {
    describe("Queries", () => {
        ...
        it("users by multiple id with 'in'", () => expect(
            sampleDb.users.find({ id: { in: ['AS', 'JS'] }}).orderBy('id').toArray().map(u => u.id))
            .toEqual(['AS', 'JS']),
        );
        ...
        it("sort by name and sex", () => expect(
                sampleDb.users
                    .orderBy('sex', 'desc')
                    .thenBy('name')
                    .map(e => e.name),
        ...
        it("sort by name and sex via order", () => expect(
            sampleDb.users
                .order([{ field: 'sex' }, { field: 'name', direction: 'desc' }])
                .map(e => e.name),
            ).toEqual(["Daenerys Targaryen", "Arya Stark", "John Snow"]),
        );
        it("find tasks by multiple values with 'in'", () => expect(
                sampleDb.tasks.find({ createdBy: { in: ['AS', 'JS'] }}).orderBy('id').map(t => t.id),
            ).toEqual([6, 7]),
        );

        it("find tasks by both indexed and non-indexed fields", () => expect(
                sampleDb.tasks.find({ createdBy: { in: ['DT'] }, assignedTo: 'JS', isDone: false }).orderBy('id').map(t => t.id),
            ).toEqual([4]),
        );

        it("non-indexable filter criteria should work", () => expect(
                sampleDb.tasks.find({ id: { gte: 4 }, assignedTo: 'AS', isDone: false }).orderBy('id').map(t => t.id),
            ).toEqual([5, 7]),
        );

        it("non-indexable and indexable criteria for the same field", () => expect(
                sampleDb.tasks.find({ createdBy: { in: ['AS', 'JS'], gt: 'B' }, isDone: false }).map(t => t.id),
            ).toEqual([6]),
        );

        it("count should work with filter", () => expect(sampleDb.tasks.find({ assignedTo: 'AS' }).count()).toEqual(2));

        it("range should work with filter", () => expect(
                sampleDb.tasks.find({ createdBy: 'DT', assignedTo: { isNull: false } }).orderBy('assignedTo').thenBy('id').range(1, 3).map(i => i.id),
            ).toEqual([1, 3, 4]),
        );
    });
    ...
    describe("Indexes and updates", () => {
        ...
        it("should handle null and undefined values", () => {
            const testDb = emptyDb.with({
                tasks: [
                    { id: 1, assignedTo: null, isDone: null },
                    { id: 2 },
                ],
            });

            expect(testDb.tasks.find({ isDone: null }).toArray().map(i => i.id)).toEqual([1]);
            expect(testDb.tasks.find({ isDone: undefined }).toArray().map(i => i.id)).toEqual([2]);
            expect(testDb.tasks.find({ isDone: { isNull: true } }).toArray().map(i => i.id)).toEqual([1, 2]);

            expect(testDb.tasks.find({ assignedTo: null }).toArray().map(i => i.id)).toEqual([1]);
            expect(testDb.tasks.find({ assignedTo: undefined }).toArray().map(i => i.id)).toEqual([2]);
            expect(testDb.tasks.find({ assignedTo: { isNull: true } }).toArray().map(i => i.id)).toEqual([1, 2]);
        });
