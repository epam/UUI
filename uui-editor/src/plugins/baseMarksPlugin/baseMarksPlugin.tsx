    ...
    const renderMark = (props: RenderMarkProps, editor: CoreEditor, next: () => any) => {
        switch (props.mark.type) {
            ...
            case 'uui-richTextEditor-span-mark':
                return (
                    <span {...props.attributes} style={props.mark.data.get('style')}>
                        {props.children}
                    </span>
                );
...
const BoldButton = (props: { editor: any }) => {
    return (
        <ToolbarButton
            isActive={props.editor.hasMark('uui-richTextEditor-bold')}
            icon={BoldIcon}
            onClick={() => props.editor.toggleMark('uui-richTextEditor-bold')}
        />
    );
};
...
const ItalicButton = (props: { editor: any }) => {
    return (
        <ToolbarButton
            isActive={(props.editor as any).hasMark('uui-richTextEditor-italic')}
            icon={ItalicIcon}
            onClick={() => props.editor.toggleMark('uui-richTextEditor-italic')}
        />
    );
};
...
const UnderlineButton = (props: { editor: any }) => {
    return (
        <ToolbarButton
            isActive={(props.editor as any).hasMark('uui-richTextEditor-underlined')}
            icon={UnderlinedIcon}
            onClick={() => props.editor.toggleMark('uui-richTextEditor-underlined')}
        />
    );
};
