...
export const linkPlugin = () => {
    const link = linkifyPlugin({
        renderComponent: (args: any) => (
            <a className={css.link} {...args} onClick={e => e.ctrlKey && window.location.replace(sanitizeUrl(`${args.href}`))}>
                {args.children}
            </a>
        ),
    });
...
export const LinkButton = (props: { editor: any }) => {
    ...
    return (
        <ToolbarButton
            isActive={(props.editor as any).hasLink()}
            icon={LinkIcon}
            onClick={() => context.uuiModals.show<string>(modalProps => <AddLinkModal {...modalProps} editor={props.editor} />).catch(() => null)}
        />
    );
};
