...
export const Sidebar: React.FC<SidebarProps> = ({ editor, plugins, isReadonly }) => {
    ...
    useEffect(() => {
        const isSidebarVisible = editor?.value.focusBlock && editor?.value.selection.isFocused && !editor?.readOnly;
    ...
    return (
        <div className={cx('slate-prevent-blur', css.sidebar)} ref={sidebarRef}>
            {flatten(plugins).map((plugin: any) =>
                plugin.sidebarButtons?.map((Button: any, index: number) => <Button editor={editor} key={`button-${index}`} />)
            )}
        </div>
    );
};
