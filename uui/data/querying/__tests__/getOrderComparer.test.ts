...
describe('getPatternPredicate', () => {
    ...
    it('by birthday', () => {
        expect(run([{ field: 'birthday' }])).toEqual([william, pete, bob, edward, jack, alice, sandra]);
    });

    it('by birthday desc', () => {
        expect(run([{ field: 'birthday', direction: 'desc' }])).toEqual([sandra, alice, jack, edward, bob, pete, william]);
    });

    it('by departmentId, then by name', () => {
        // Null and undefined should be treated as equal
        expect(run([{ field: 'departmentId' }, { field: 'name' }])).toEqual([jack, william, edward, pete, bob, sandra, alice]);
    });

    it('by departmentId desc, then by name', () => {
        expect(run([{ field: 'departmentId', direction: 'desc' }, { field: 'name' }])).toEqual([alice, bob, sandra, edward, pete, jack, william]);
    });
});
