...
function FiltersToolbarItemImpl(props: FiltersToolbarItemProps) {
    ...
    const getTogglerValue = () => {
        ...
        switch (props.type) {
            ...
            case 'datePicker': {
                return { selection: currentValue ? [dayJsHelper.dayjs(currentValue).format(props.format || defaultFormat)] : currentValue };
            }
            case 'rangeDatePicker': {
                ...
                const currentValueFrom = currentValue?.from
                    ? dayJsHelper.dayjs(currentValue?.from).format(props.format || defaultFormat)
                    : i18n.filterToolbar.rangeDatePicker.emptyPlaceholderFrom;
                const currentValueTo = currentValue?.to
                    ? dayJsHelper.dayjs(currentValue?.to).format(props.format || defaultFormat)
