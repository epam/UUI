...
export interface RangeDatePickerBodyProps<T> extends CommonDatePickerBodyProps, Pick<RangeDatePickerProps, 'preventEmptyToDate' | 'preventEmptyFromDate' | 'initialViewDate'>, IControlled<RangeDatePickerBodyValue<T>> {
...
const getInitialViewDate = (selectedDate: RangeDatePickerValue, initialViewDate: string | undefined): RangeDatePickerValue => {
    return selectedDate.from || selectedDate.to ? selectedDate : {
        from: initialViewDate || null,
        to: null,
    };
};
...
function RangeDatePickerBodyComp(props: RangeDatePickerBodyProps<RangeDatePickerValue | null>, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element {
    const { value: _value, filter, initialViewDate: _initialViewDate } = props;
    const {
    ...
    const initialViewDate = getInitialViewDate(selectedDate, _initialViewDate);
    ...
    const [month, setMonth] = useState(() => {
        return getDisplayedMonth(initialViewDate, inFocus);
    });
    ...
    useLayoutEffectSafeForSsr(() => {
        const monthToSet = getDisplayedMonth(initialViewDate, inFocus);
