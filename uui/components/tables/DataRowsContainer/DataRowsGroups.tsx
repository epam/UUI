...
function DataRowsGroup<TItem, TId>({
    ...
    const childrenPinnedTop = row.isPinned ? (top + (rowRef.current?.clientHeight ?? 0) - 1) : top;
    return ( 
        <div className={ css.group } key={ row.rowKey }>
            <div className={ row.isPinned ? css.stickyHeader : css.header } style={ { zIndex: row.depth + 10, top: top - 1 } } ref={ rowRef }>
                {renderRow(row)}
            </div>
            {childRows.length > 0 && (
                <div>
