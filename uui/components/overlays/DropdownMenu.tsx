...
const DropdownMenuContainer = (props: IDropdownMenuContainer) => {
    ...
    const menuItems: HTMLElement[] = menuRef.current
        ? Array.from(menuRef.current.querySelectorAll(`[role="menuitem"]:not(.${uuiMod.disabled})`))
        : [];
...
export const DropdownMenuButton = (props: IDropdownMenuItemProps) => {
    ...
    const itemClassNames = cx(props.cx, css.itemRoot, isDisabled && uuiMod.disabled, isSelected && uuiMod.selected);
...
export const DropdownSubMenu = (props: IDropdownSubMenu) => {
    return (
        <Dropdown
            ...
            renderBody={props => <DropdownMenuBody closeOnKey={DropdownControlKeys.LEFT_ARROW} {...props} />}
