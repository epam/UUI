...
export function createInlineStyleLinkButton({ style, icon, caption }: DraftInlineStyleButton) {
    return class InlineStyleButton extends React.Component<DraftButtonProps> {
        toggleStyle = (event: React.MouseEvent<HTMLDivElement>) => {
            this.props.onValueChange(RichUtils.toggleInlineStyle(this.props.value, style));
        };
        ...
        render() {
            return (
                <div onMouseDown={this.preventBubblingUp}>
                    <IconButton onClick={this.toggleStyle} icon={icon} color={this.styleIsActive() ? 'sky' : 'night600'} />
                </div>
            );
        }
    };
