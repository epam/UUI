...
const getPersons = cached('persons', async () => {
    ...
    // Separate Chance instance to fix yearly inconsistencies for `.birthday()` api
    // and to not affect other data generated by `.year()` api by main Chance instance
    const cBirthYear = new Chance(1);
    ...
    for (let n = 0; n < size; n++) {
        ...
        persons.push({
            ...
            birthDate: (c.birthday({ year: cBirthYear.year({ min: 1925, max: 2025 }) }) as Date).toDateString(),
