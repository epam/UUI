...
export class AppHeader extends React.Component {
    ...
    renderBurger = () => {
        ...
        return (
            <>
                <BurgerButton caption="Home" link={{ pathname: '/' }} clickAnalyticsEvent={() => this.sendEvent('Welcome')} />
                <BurgerButton
                    caption="Documents"
    ...
    renderThemeSwitcher = () => {
        ...
        return (
            <Dropdown
                renderTarget={props => <MainMenuButton isDropdown caption="Choose theme" {...props} />}
                renderBody={props => (
                    <DropdownMenuBody {...props}>
                        <DropdownMenuButton
                            caption="Promo"
                            isSelected={thisTheme === 'promo'}
                            iconPosition="right"
                            onClick={() => this.setTheme('promo')}
                        />
                        <DropdownMenuButton
                            caption="Loveship"
                            isSelected={thisTheme === 'loveship'}
                            iconPosition="right"
                            onClick={() => this.setTheme('loveship')}
                        />
        ...
        return [
            {
                ...
                render: () => <Burger renderBurgerContent={this.renderBurger} logoUrl="/static/logo.svg" key="burger" />,
            },
            {
                id: 'logo',
                priority: 100499,
                render: () => (
                    <MainMenuLogo link={{ pathname: '/' }} onClick={() => this.sendEvent('Welcome')} logoUrl="/static/logo.svg" key="logo" />
                ),
            },
            {
                id: 'documents',
            ...
            window.location.host.includes('localhost') && {
                ...
                priority: 1,
                render: () => (
                    <MainMenuButton caption="Sandbox" link={{ pathname: '/sandbox' }} isLinkActive={pathName === '/sandbox'} key="sandbox" />
                ),
            },
    ...
    render() {
        return <MainMenu cx={css.root} items={this.getMainMenuItems()}></MainMenu>;
    }
}
