...
export const PersonsTable = (props: PersonsTableProps) => {
    ...
    const renderRow = (props: DataTableRowProps<PersonTableRecord, PersonTableRecordId>) => {
        ...
        return <DataTableRow key={String(props.id)} {...props} columns={cols} background={isGroup ? 'night50' : 'white'} />;
    };
    ...
    const onConfigurationButtonClick = () => {
        uuiModals
            .show<ColumnsConfig>(modalProps => (
                <ColumnsConfigurationModal {...modalProps} columns={personColumns} columnsConfig={config} defaultConfig={defaultConfig} />
            ))
        ...
        <>
            ...
            {props.view.getListProps().exactRowsCount !== 0 && (
                <div className={css.listContainer} style={{ minHeight: `${estimatedHeight}px` }}>
                    <div ref={listContainerRef} role="rowgroup" style={{ marginTop: offsetY }} children={getRows()} />
                </div>
            )}
            <DataTableRow columns={summaryColumnsSync} cx={css.stickyFooter} id="footer" rowKey="footer" index={100500} value={props.summary} />
        </>
    );
