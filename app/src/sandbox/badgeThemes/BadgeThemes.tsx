...
export function BadgeThemes() {
    ...
    const mapThemes:Record<typeof appTheme.current, string> = {
        'uui-theme-loveship': 'Loveship',
        'uui-theme-vanilla_thunder': 'Loveship Dark',
    ...
    return (
        <div style={ { display: 'flex', flexDirection: 'column', gap: '48px 0', backgroundColor: appTheme.current === 'uui-theme-loveship_dark' ? '#1c1c1c' : 'transparent' } }>
            ...
            <FlexCell>
                <Text fontSize="24" font="semibold">{ mapThemes[appTheme.current] }</Text>
                <div style={ {
                    padding: '24px',
                    display: 'grid',
                    gridTemplateColumns: 'repeat(7, max-content)',
