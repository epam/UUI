...
interface CustomVirtualListProps {
    rows: DataRowProps<City, string>[];
    estimatedHeight: number;
    offsetY: number;
    listContainer: React.MutableRefObject<HTMLUListElement>;
}
...
const CustomVirtualList = ({
    ...
    listContainer,
    offsetY
}: React.PropsWithChildren<CustomVirtualListProps>) => {
    const getRows = () => {
    ...
    return (
        <div ref={ scrollContainer } onScroll={ handleScroll } className={ css.mainContainer }>
            ...
            <CustomVirtualList
                ...
                rows={ citiesDataSource.useView(value, onValueChange).getVisibleRows() }
            />
        </div>
    );
}