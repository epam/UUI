...
const badgeDoc = new DocBuilder<BadgeProps & BadgeMods>({ name: 'Badge', component: Badge })
    .prop('color', { renderEditor: (editable: any, examples) => <ColorPicker colors={ examples.map(i => ({ value: i })) } { ...editable } />, examples: allEpamBadgeSemanticColors })
    .implements([iconOptionsDoc, dropdownTogglerDoc, onClickDoc, basicPickerTogglerDoc])
