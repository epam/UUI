...
export default function TypesExample() {
    ...
    const statusDot = (color: string) => (
        <span className={ css.dot } style={ {backgroundColor: color} } />
    );
        ...
        return (
            <Panel background='white' shadow>
                { dropdownMenuItems.map((item) => (
                    <DropdownMenuButton key={ item.id } caption={ item.caption }
                    icon={ () => statusDot(item.color) } onClick={ () => {
            ...
            <Dropdown
                ...
                renderTarget={ (props) =>
                    <Badge { ...props } dropdownIcon={ navigationDownIcon } dropdownIconPosition="right"
                        color='gray30' icon={ () => statusDot(selectedItem.color) } fill='semitransparent'
