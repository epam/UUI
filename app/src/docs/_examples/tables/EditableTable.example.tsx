...
const demoItems: ToDoItem[] = [
    ...
    { ...blankItem, id: id++, name: 'Find better ways to add/remove rows', dueDate: '01-09-2022', priority: 2 },
    {
        ...blankItem,
        id: id++,
        name: 'Finalize the "Project" table demo',
        comments: 'We first need to build the add/remove rows helpers, and rows drag-n-drop',
    },
...
export default function EditableTableExample() {
        ...
        () =>
            [
                ...
                },
                {
                    ...
                    caption: 'Done',
                    renderCell: props => (
                        <DataTableCell {...props.rowLens.prop('isDone').toProps()} renderEditor={props => <Checkbox {...props} />} {...props} />
                    ),
                    textAlign: 'center',
                    fix: 'left',
                    width: 80,
    ...
    const renderRow = useCallback((props: DataTableRowProps<ToDoItem, number>) => {
        return <DataTableRow {...props} background={props.value.isDone ? 'gray5' : 'white'} />;
    }, []);
