...
import { Button, Dropdown, DropdownContainer, FlexCell, Slider, VerticalTabButton } from "@epam/promo";
...
export default function BasicAdaptivePanelExample() {
    ...
    const renderItem = (item: AdaptiveItemProps<{data?: {caption: string}}>) => {
        return (
            <div><Button cx={ css.itemWithMargins } caption={ item.data.caption } /></div>
        );
    };
    ...
    const items: AdaptiveItemProps<{data?: {caption: string}}>[] = [
        ...
        { id: '5', render: (item, hiddenItems) => <Dropdown
                ...
                renderBody={ () => <DropdownContainer>{ hiddenItems.map(i => <VerticalTabButton caption={ i.data.caption } />) }</DropdownContainer> }
            />,
