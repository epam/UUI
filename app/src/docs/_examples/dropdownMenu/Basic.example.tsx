...
export default function BasicDropdownMenuExample() {
    ...
    const getSubmenuLayer = () =>
        layer.map(item => (
            <DropdownMenuButton caption={item.caption} onClick={() => setLayerHandler(item.id, item.checked)} isActive={item.checked} />
        ));
    ...
    return (
        <>
            <Dropdown
                ...
                renderTarget={props => (
                    <Avatar img={'https://avatars.dicebear.com/api/human/avatar12.svg?background=%23EBEDF5&radius=50'} size={'36'} {...props} />
                )}
            />
            <ControlGroup>
