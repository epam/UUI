...
import {
    DndActor, IEditable, cx, uuiDndState, DropParams, getOrderBetween, DndActorRenderParams,
} from '@epam/uui-core';
import {
...
export class DndSection extends React.Component<DndSectionProps> {
    ...
    onItemChange = (value: Partial<SectionItem>) => {
        this.props.onValueChange({ ...this.props.value, ...value });
    };

    onFoldToggle = () => {
        this.props.onValueChange({ ...this.props.value, isFolded: !this.props.value.isFolded });
    };
            ...
            <DndActor
                ...
                render={ (props) => (
                    <Section
                        ...
                        onMaterialDropFromSection={ this.props.onMaterialDropFromSection }
...
const Section = React.forwardRef(({
    ...
    sortedCriteria,
    sortedMaterials,
    onMaterialDropFromSection,
    onFoldToggle,
    ...props
}: DndActorRenderParams & {
    item: SectionItem;
    sortedCriteria: CriterionItem[];
    sortedMaterials: MaterialItem[];
    onMaterialDropFromSection: (data: DropParams<MaterialItem, MaterialItem>) => void;
    onItemChange: (value: Partial<SectionItem>) => void;
    onFoldToggle: () => void;
}, ref) => {
    ...
    React.useEffect(() => {
        ...
        const elem = rowRef.current;
        elem?.addEventListener('pointerup', onMouseUpRow);
        return () => {
            elem?.removeEventListener('pointerup', onMouseUpRow);
        };
    }, [onFoldToggle, props.dragData]);

    return (
        <div ref={ ref as React.ForwardedRef<any> } { ...props.eventHandlers } className={ cx(css.dragElement, props.classNames) }>
                            ...
                            { sortedCriteria.map((criterion, index) => (
                                <DndCriterion
                                    ...
                                    onValueChange={ (newValue) =>
                                        onItemChange({ ...item, criteria: item.criteria.map((i) => (i.id === newValue.id ? newValue : i)) }) }
                                />
                            )) }
                        </div>
                        <div className={ css.materialsSection }>
                            ...
                            { sortedMaterials.map((material, index) => (
                                <DndMaterial
                                    ...
                                    onDropFromSection={ onMaterialDropFromSection }
                                    onValueChange={ (newValue) =>
                                        onItemChange({ ...item, materials: item.materials.map((i) => (i.id === newValue.id ? newValue : i)) }) }
                                />
                            )) }
                        </div>
                    </>
